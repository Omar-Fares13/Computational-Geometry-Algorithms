...
namespace CGUtilities
{
    ...
    public class Point : ICloneable
    {
        ...
        {
            if (Math.Abs(X - other.X) > Constants.Epsilon)
            {
                return X.CompareTo(other.X); // Compare by X-coordinate first
            }
            return Y.CompareTo(other.Y); // If X is equal, compare by Y-coordinate
        }
    }
}
